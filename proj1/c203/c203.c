...
void Queue_Init( Queue *queue ) {
    ...
    for (int i = 0; i < QUEUE_SIZE; i++)    // vsechny hodnoty v poli nastavim na *
        queue->array[i] = '*';
    
    queue->firstIndex = 0;
    queue->freeIndex = 0;   // nastavim indexy na 0
}
...
int nextIndex( int index ) {
	...
	return ++index % QUEUE_SIZE;    // vraceni nasledujiciho prvku v poli
}
...
int Queue_IsEmpty( const Queue *queue ) {
    ...
    return queue->firstIndex == queue->freeIndex;   // pokud je fronta prazdna, vraci se nenulova hodnota, jinak se vraci 0
}
...
int Queue_IsFull( const Queue *queue ) {
    ...
    return queue->firstIndex == nextIndex(queue->freeIndex);    // pokud je fronta plna, vraci se nenulova hodnota, jinak se vraci 0
}
...
void Queue_Front( const Queue *queue, char *dataPtr ) {
    ...
    if(Queue_IsEmpty(queue))
        Queue_Error(QERR_FRONT);    // pokud je fronta prazdna, volam error
    else
        *dataPtr = queue->array[queue->firstIndex]; // vraceni znaku na zacatku fronty
}
...
void Queue_Remove( Queue *queue ) {
    ...
    if(Queue_IsEmpty(queue))
        Queue_Error(QERR_REMOVE);   // pokud je fronta prazdna, vracim error
    else
        queue->firstIndex = nextIndex(queue->firstIndex); // jinak presouvam prvni prvek na nasledujici prvek
}
...
void Queue_Dequeue( Queue *queue, char *dataPtr ) {
    ...
    if(Queue_IsEmpty(queue))
        Queue_Error(QERR_DEQUEUE);  // pokud je fronta prazdna, vracim error
    else
    {
        Queue_Front(queue, dataPtr);
        Queue_Remove(queue);    // vracim znak ze zacatku fronty a nasledne ho mazu
    }
}
...
void Queue_Enqueue( Queue *queue, char data ) {
    ...
    if(Queue_IsFull(queue))
        Queue_Error(QERR_ENQUEUE);  // pri plne fronte volam error
    else
    {
        queue->array[queue->freeIndex] = data;
        queue->freeIndex = nextIndex(queue->freeIndex); // na volny index vkladam data a posouvam volny index na dalsi index 
    }
}
